...
def copia_trecho_rgb(image, xini,xfim,yini,yfim,zini,zfim):
    deltax = xfim-xini
    deltay = yfim-yini
    deltaz = zfim-zini
    if verbose:
...
def copia_trecho_gs(image, xini,xfim,yini,yfim):
    deltax = xfim-xini
    deltay = yfim-yini
    
    if verbose:
        print('delta:',deltax,deltay)
        print(xini,xfim,yini,yfim)
    ret=np.zeros(deltax*deltay)
    ret = ret.reshape(deltay,deltax)
    ret[:,:] = image[yini:yfim,xini:xfim]
    return ret.astype(np.uint8)
...
def run_gs():
    ...
    for tile in range(grid_dims[0]*grid_dims[1]):
        if tile != grid_dims[0]*grid_dims[1] - 1:
            myrank = (tile * grid_dims[0]) % (grid_dims[0] * grid_dims[1] -1)
        else:
            myrank = tile 
        myrank_2D = myrank % (grid_dims[1]*grid_dims[0])
