...
static int parse_args(int key, char *arg, struct argp_state *state){
    ...
    if(key == 'f'){
        ...
        printf("%s",arg);
        len = strlen(arg);
        program_args.img_ext = (char*) malloc(len *sizeof(char));
        strncpy(program_args.img_ext, arg, len);
        upcase_string(program_args.img_ext);
    }
    return 0;
}
...
int main(int argc, char **argv){
    struct argp_option args_opts[] = {
        ...
        {0,'i',"FILENAME",0,"Input file name"},
        {0,'o',"FILENAME",0,"Output file name"},
        {0,'e', "TYPE", 0, "Input and output format"},
        {0}
    };
